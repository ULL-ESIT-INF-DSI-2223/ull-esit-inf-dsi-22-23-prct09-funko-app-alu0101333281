...
import { createFunko, deleteFunko, readFunko, modifyFunko, userDir, getFunkoAtributte } from '../src/functions.js';
...
describe('createFunko', () => {
  ...
  it('should create a new Funko', () => {
    const result = createFunko(user, id, name, description, type, genre, franchise, number, exclusive, specialFeatures, marketValue);
  ...
  it('should not create a new Funko with invalid type', () => {
    ...
    const result = createFunko(user, id, name, description, invalidType, genre, franchise, number, exclusive, specialFeatures, marketValue);
    expect(result).to.be.true;
    expect(fs.existsSync(userDir + user + '/' + name)).to.be.true;
  });
});
...
describe('modifyFunko()', function() {
  ...
  it('Debería modificar un funko existente correctamente', function() {
    ...
    const result = modifyFunko(user, id, name, newname, description, type, genre, franchise, number, exclusive, specialFeatures, marketValue);
...
describe('getFunkoAtributte', () => {
  ...
  it('should return the value of the requested attribute', () => {
    ...
    expect(getFunkoAtributte('funko1', 'user1', 'description')).to.equal(undefined);
    ...
    expect(getFunkoAtributte('funko1', 'user1', 'franchise')).to.equal(undefined);
    expect(getFunkoAtributte('funko1', 'user1', 'number')).to.equal(undefined);
    expect(getFunkoAtributte('funko1', 'user1', 'exclusive')).to.equal(undefined);
    expect(getFunkoAtributte('funko1', 'user1', 'specialFeatures')).to.equal(undefined);
    expect(getFunkoAtributte('funko1', 'user1', 'marketValue')).to.equal(undefined);
  });

  it('should return undefined when funko file does not exist', () => {
    expect(getFunkoAtributte('nonexistentfunko', 'user1', 'name')).to.be.undefined;
  });

  it('should return undefined when user directory does not exist', () => {
    expect(getFunkoAtributte('funko1', 'nonexistentuser', 'name')).to.be.undefined;
  });

  it('should return undefined when attribute does not exist', () => {
    expect(getFunkoAtributte('funko1', 'user1', 'nonexistentattribute')).to.be.undefined;
  });
});
